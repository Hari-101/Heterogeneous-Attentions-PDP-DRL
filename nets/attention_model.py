# -*- coding: utf-8 -*-
"""Attention_Model_cap_pdp.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/12ffOAj7WP4DHbhYO8hS7ukwmVngqARtZ
"""
...
class AttentionModel(nn.Module):
    ...
    def __init__(self,
        ...
        #AUGMENT#
        self.is_cap_pdp = problem.NAME == 'cap_pdp'   #***NEW***#
        ...
        if self.is_vrp or self.is_orienteering or self.is_pctsp:
            ###*** Wont the remaining capacity signal get stifled by the 128 dimensional embedding? ***###
        ...
        #AUGMENT#
        elif self.is_cap_pdp:   #***NEW***#
            step_context_dim = embedding_dim + 1  #(emb_dim, remaining capacity of resource  )
            node_dim = 3    #(x,y, demand)
            self.init_embed_depot = nn.Linear(2, embedding_dim)     #No order quantity at depot
            #--- Do we need this as the route will begin at depot? (Like VRP)
            self.W_placeholder = nn.Parameter(torch.Tensor(2 * embedding_dim))
            self.W_placeholder.data.uniform_(-1, 1)  # Placeholder should be in range of activations
            #---
            self.init_embed_pick = nn.Linear(node_dim * 2 , embedding_dim) #(x_p, y_p, dem, x_d, y_d, -dem)
            self.init_embed_delivery = nn.Linear(node_dim, embedding_dim) #(x_d, y_d, -dem)
        else:  # TSP
    ...
    def forward(self, input, return_pi=False):
        ...
        if self.checkpoint_encoder and self.training:  # Only checkpoint if we need gradients
            embeddings, _ = checkpoint(self.embedder, self._init_embed(input))    #CHANGE _init_embed()
        else:
            embeddings, _ = self.embedder(self._init_embed(input))

        _log_p, pi = self._inner(input, embeddings)   #CHANGE _inner()

        cost, mask = self.problem.get_costs(input, pi)    #CHANGE problem.get_costs(input, pi)--- Would this function change?
    ...
    #Make sure we get n_batch x n_nodes x n_embeddings at output
    #AUGMENT#
    def _init_embed(self, input):
        ...
        if self.is_cap_pdp:
            ...
            input['loc'][:, n_loc // 2:, :], input['demand'][:, n_loc // 2:, None]], -1)    #Check if adding None is correct?
            ...
            embed_pick = self.init_embed_pick(feature_pickup)
    ...
    def _inner(self, input, embeddings):
        ...
        state = self.problem.make_state(input)    #Update code for problem.make_state()
    ...
    #AUGMENT#
    ###*** Needs to be modified for capacitated_pdp ! ***###
